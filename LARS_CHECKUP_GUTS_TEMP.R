...
grass_ep$co2_signal_strength=grass_ep$co2_signal_strength_7500_mean
...
####Prefilter
pine_ep$SHF_3_1_1=NA
grass_ep$SHF_2_1_1=NA
###MERGING#####
...
summary_ep$co2_signal_strength[summary_ep$co2_signal_strength < 70 ] = NA
summary_ep$SWIN_1_1_1[summary_ep$SWIN_1_1_1 < -10] = NA
summary_ep$LWIN_1_1_1[summary_ep$LWIN_1_1_1 < 200] = NA
###############
...
main_vars=c('H','LE','co2_flux','ALBEDO'
            ,'SWIN_1_1_1','SWOUT_1_1_1','LWIN_1_1_1','LWOUT_1_1_1'
            ,'air_temperature','RH','C_RAIN'
            ,'wind_speed','flowrate_mean','co2_signal_strength'
            ,'u.',"RN_1_1_1",'SWDIF_1_1_1','TA_2M')
...
dat3=gather_(summary_ep,'COMP','VALUE',index_vars)
dat3=dat3[order(dat3$DOY),]
dat3$METCOMP=paste0(sapply(strsplit(dat3$COMP,'I'),'[',1),'I')
dat3$SENSNUM=substr(sapply(strsplit(dat3$COMP,'I'),'[',2),start = 1,stop = 1)

##########
mycolors3=brewer.pal(8,"Set2")
my.settings2=list(
  superpose.symbol=list(col=mycolors2,alpha=0.8)
  ,superpose.line=list(col=mycolors2,alpha=0.8,lwd=2)
  ,strip.background=list(col=brewer.pal(3,'Greys'))
...
plot3=xyplot(VALUE~DOY|PFT+METCOMP
             ,data=dat3,groups=SENSNUM,layout=c(3,2)
             ,scales=list(y=list(relation='free')
                          ,x=list(at=myat
                                  ,labels=mylabels
                                  ,alternating=3
